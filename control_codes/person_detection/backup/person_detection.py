...
def in_exclude_list(x1,x2,y1,y2):
	# check if the detected motion is caused by visible light
	# Find UV locations, 
	# check with the detected motion boxes (x1,x2,y1,y2), if they match then return True

	S = shared_variables.scored_spots.reset_index()
	df_UV = S[S['score']==-1]

	print('Iin exclide list: x1,x2,y1,y2', x1,x2,y1,y2)
	#print('df_UV', df_UV)
	...
	print('in person_detection: p_UV: ', np_UV)

	for i in range(np_UV.shape[0]):
		x_uv,y_uv = np_UV[i,0],np_UV[i,1]

		m = shared_variables.Coverage_size
		a = 10


		if x_uv-a<x1 and x2<x_uv+a+m/2 and y_uv-a<y1 and y2<y_uv+a+m/2:
...
class person_detection:
	...
	def get_all_people(self, frame1, frame2):
		...
		for contour in contours:
			...
			#if in_exclude_list(x1,x2,y1,y2) == False:
