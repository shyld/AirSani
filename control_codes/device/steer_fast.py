...
#### Functions for two step steering
	...
	def two_step_steer(self, LED, x,y): # the light will automatically turn on
		...
		# Turn off light (without changing the location)
		...
		time.sleep(10) # to be removed in the product code
		self.report_light_status(LED, 'on')

		# Update the current position of LEDs in the internal class variables
		...
		try:
			...
			print(self.find_nearest_loc(x,y))            
